...
def save_data(folder_name, voltaje, tiempo, max_levels):
    with open(folder_name, 'w', newline="") as csv_file:
        a = csv.writer(csv_file, delimiter=',')
        level_counter = 0
        while level_counter < max_levels:
            a.writerow((voltaje[level_counter], tiempo[level_counter]))
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            voltaje = voltaje + [round(adc_0, 2)]
            tiempo = tiempo + [round(count, 2)]
   
            save_data('prueba.csv', voltaje, tiempo, len(tiempo))
            count = count + 0.2
